...
const groupedVms = computed(() => {
  const groups = {}; const templates = []; const otherVMs = []
  for (const vm of vms.value) {
    if (vm.hardware_details && vm.hardware_details.template === 1) { templates.push(vm) } else { otherVMs.push(vm) }
  }
  for (const template of templates) { groups[template.name] = { template: template, clones: [] } }
  const unassignedVMs = []
  for (const vm of otherVMs) {
    const description = vm.hardware_details?.description || ''; const match = description.match(/Cloned from template: (.*)/)
    if (match && match[1] && groups[match[1].trim()]) { groups[match[1].trim()].clones.push(vm) } else { unassignedVMs.push(vm) }
  }
  if (unassignedVMs.length > 0) { groups['Other VMs'] = { template: null, clones: unassignedVMs } }
  return groups
})
...
async function handleBulkAction(action) {
  ...
  try {
    ...
    // Refresh the VM list after a short delay to allow statuses to update
